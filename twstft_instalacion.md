...
Programas: `lee_satres.py` v0.5 · `procesa_sesion.py` v1.7 · `calcula_diff.py` v1.3 · `envia_zabbix.py` v0.1
...
|---|---|---|
| `lee_satres.py` | 0.5 | Receptor UDP. Escucha el puerto 3020 y graba los mensajes del módem SATRE en ficheros raw diarios. |
...
| `calcula_diff.py` | 1.3 | Calcula las diferencias de tiempo entre laboratorios a partir de los ficheros ITU y genera ficheros `.dat` y gráficas HTML y PNG. |
| `envia_zabbix.py` | 0.1 | Calcula las diferencias de tiempo y las envía a un servidor Zabbix via `zabbix_sender` para monitorización y visualización en Grafana. |
...
Configuración del servidor Zabbix para el envío de datos desde `envia_zabbix.py`:

```ini
[zabbix]
servidor    = zabbix.roa.es   # hostname o IP del servidor Zabbix
puerto      = 10051           # puerto Zabbix trapper (defecto 10051)
host        = TWSTFT          # nombre del host en Zabbix
item_prefix = twstft          # prefijo de los item keys
activo      = no              # si/no — activar envío a Zabbix
```

Los item keys se generan automáticamente como `<prefix>.<loc>.<rem>`,
por ejemplo `twstft.roa01.ptb05`.

### 5.4 Parámetros de descarga en [lab XXXX]

Cada sección de laboratorio puede incluir la URL de descarga de sus ficheros ITU:

```ini
[lab PTB05]
...
itu_url   = ftp://usuario:password@ftp.ptb.de/pub/twstft
itu_mayus = no    # si/no — nombre del fichero en mayúsculas
```

Cuando `calcula_diff.py` o `envia_zabbix.py` se ejecutan con `--config` y
el laboratorio remoto no tiene directorio local especificado, descargan
automáticamente el fichero ITU desde esta URL.

### 5.5 Verificar los laboratorios activos
...
ficheros ITU. Genera ficheros `.dat` con las diferencias slot a slot y
gráficas HTML (plotly) y PNG (matplotlib) con todos los laboratorios.

`twstft.ini` es opcional: si se proporciona con `--config`, permite la
descarga automática de ficheros ITU remotos via FTP/HTTP usando los
parámetros `itu_url` e `itu_mayus` de cada sección `[lab XXXX]`.
...
| `--config` | `RUTA` | Ruta a `twstft.ini`. Opcional. Activa la descarga automática de ficheros ITU remotos via FTP/HTTP. |
| `--debug` | — | Log detallado en consola. |
| `--version` | — | Muestra la versión del programa y termina. |
...
> El directorio en `--remoto` es opcional si se usa `--config` con `itu_url`
> configurado en la sección `[lab XXXX]` correspondiente.
...
Calcula las diferencias de tiempo `UTC(local) - UTC(lab_remoto)` entre el
laboratorio local y uno o varios laboratorios remotos, a partir de sus
ficheros ITU, y envía los resultados a un servidor Zabbix via `zabbix_sender`.

Cada slot se envía como un dato independiente con su **timestamp real**
(derivado del MJD + STTIME), lo que permite a Zabbix y Grafana representar
la serie temporal completa y configurar alertas sobre valores fuera de norma.

La configuración del servidor Zabbix se lee de la sección `[zabbix]` del
fichero `twstft.ini`. El envío solo se realiza si `activo = si`.

**Item key generado:**

```
<item_prefix>.<loc_lower>.<rem_lower>
Ejemplo: twstft.roa01.ptb05
```

**Requisito del sistema:** `zabbix_sender` debe estar instalado:
...
|---|---|---|
| `--local` | `RUTA` | Fichero ITU del laboratorio local del día más reciente. **Obligatorio.** |
| `--remoto` | `LAB[:DIR]` | Laboratorio remoto y directorio de sus ficheros ITU. El directorio es opcional si `itu_url` está configurado en el ini. Repetir para varios labs. **Al menos uno obligatorio.** |
| `--ventana` | `N` | Número de días a procesar hacia atrás. Defecto: 1. |
| `--config` | `RUTA` | Ruta a `twstft.ini`. Necesario para leer la configuración Zabbix y para la descarga automática de ficheros remotos. Defecto: `twstft.ini` en el mismo directorio del script. |
| `--debug` | — | Log detallado en consola. |
| `--version` | — | Muestra la versión del programa y termina. |
...
```bash
# Enviar diferencias ROA-PTB y ROA-SP del día actual
python3 envia_zabbix.py \
    --local  /home/tw/satres/448/itu/twroa61.199 \
    --remoto PTB05:/home/tw/satres/itu_remoto/ptb \
    --remoto SP01:/home/tw/satres/itu_remoto/sp \
    --config /home/tw/twstft/twstft.ini

# Con descarga automática via FTP (sin directorio local)
python3 envia_zabbix.py \
    --local   /home/tw/satres/448/itu/twroa61.199 \
    --remoto  PTB05 \
    --remoto  SP01 \
...
# Ventana de 5 días desde cron
python3 envia_zabbix.py \
    --local   /home/tw/satres/448/itu/twroa61.199 \
    --remoto  PTB05 --remoto SP01 --remoto USNO01 \
    --ventana 5 \
    --config  /home/tw/twstft/twstft.ini
```
...
```cron
# Enviar a Zabbix a las 01:00 con los datos del día anterior
0 1 * * * /home/tw/twstft/venv/bin/python3 \
    /home/tw/twstft/envia_zabbix.py \
    --local /home/tw/satres/448/itu/$(date -d yesterday +twroa%y.%j) \
    --remoto PTB05 --remoto SP01 \
    --config /home/tw/twstft/twstft.ini
```
...
**Dependencias del sistema** (adicionales a Python):

| Paquete | Instalación | Necesario para |
|---|---|---|
| `zabbix-sender` | `sudo dnf install zabbix-sender` | `envia_zabbix.py` |
